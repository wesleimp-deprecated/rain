...
var Command = &cli.Command{
	Name:    "push",
	Usage:   "Push the current project",
	Aliases: []string{"p"},
	Flags: []cli.Flag{
	...
	Action: func(c *cli.Context) error {
		start := time.Now()
		log.Infof(color.New(color.Bold).Sprint("pushing..."))
		...
		err = ctrlc.Default.Run(ctx, func() error {
			for _, pipe := range pipeline.PushPipeline {
		...
		if err != nil {
			return errors.Wrap(err, color.New(color.Bold).Sprintf("push failed after %0.2fs", time.Since(start).Seconds()))
		}

		log.Infof(color.New(color.Bold).Sprintf("push succeeded after %0.2fs", time.Since(start).Seconds()))
		return nil
	},
}
